...
export default function CommandPalette() {
  ...
  const results = useMemo(() => {
    ...
    return items.filter(p => {
      ...
      return t.includes(q) || e.includes(q) || c.includes(q) || cat.includes(q)
    }).slice(0, 50)
  }, [items, query])
  ...
  const highlight = (text: string, q: string) => {
    ...
    return (
      <>
        {before}
        <mark className="bg-orange-200/60 dark:bg-orange-900/50 rounded px-0.5 text-stone-900 dark:text-stone-100">{match}</mark>
        {after}
      </>
    )
  }
      ...
      >
        ...
        >
          <div className="relative border-b border-stone-200 dark:border-stone-700">
            ...
            />
              <kbd className="ml-2 rounded border px-1.5 py-0.5 text-[10px] text-stone-500 dark:text-stone-400 border-stone-300 dark:border-stone-600">⌘K</kbd>
            </div>
          </div>
          <Cmdk.List className="max-h-96 overflow-auto py-2" style={{ animation: 'fadeIn 120ms ease-out' }}>
            <Cmdk.Empty className="px-4 py-6 text-sm text-stone-500">No results found.</Cmdk.Empty>
            ...
            {query === '' && (
              <Cmdk.Group heading="Quick actions">
                ...
                >
                  ...
                  <div>
                    <div className="font-medium text-stone-900 dark:text-stone-100">Open full search</div>
                    <div className="text-xs text-stone-500">Use advanced results and filters</div>
                  </div>
                </Cmdk.Item>
                <Cmdk.Item
                  value="go-home"
                ...
                >
                  ...
                  <div>
                    <div className="font-medium text-stone-900 dark:text-stone-100">Go to homepage</div>
                    <div className="text-xs text-stone-500">Browse latest posts</div>
                  </div>
                </Cmdk.Item>
                <Cmdk.Item
                  value="contact"
                ...
                >
                  ...
                  <div>
                    <div className="font-medium text-stone-900 dark:text-stone-100">Contact NextRows</div>
                    <div className="text-xs text-stone-500">Get in touch with our team</div>
                  </div>
                </Cmdk.Item>
                <Cmdk.Item
                  value="subscribe"
                ...
                >
                  ...
                  <div>
                    <div className="font-medium text-stone-900 dark:text-stone-100">Subscribe via RSS</div>
                    <div className="text-xs text-stone-500">Follow updates in your reader</div>
                  </div>
                </Cmdk.Item>
              </Cmdk.Group>
            )}
              ...
              {loading && items.length === 0 && (
                <div className="space-y-2 px-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="h-10 rounded-md bg-stone-100 dark:bg-stone-800 animate-pulse" />
                  ))}
                </div>
              )}
              {!loading && results.map(p => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="font-medium text-stone-900 dark:text-stone-100 truncate">{highlight(p.title, query)}</div>
                    <div className="text-xs text-stone-500 mt-0.5 flex items-center gap-2">
                      <span className="inline-flex items-center rounded-full border border-stone-300 dark:border-stone-600 px-2 py-0.5 text-[10px] uppercase text-stone-600 dark:text-stone-300">
                        {p.category ? p.category.replace('-', ' ') : 'Article'}
                      </span>
          ...
          <div className="flex items-center justify-between border-t border-stone-200 dark:border-stone-700 px-4 py-2 text-[11px] text-stone-500">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-1"><kbd className="rounded border px-1.5 py-0.5 border-stone-300 dark:border-stone-600">↑</kbd><kbd className="rounded border px-1.5 py-0.5 border-stone-300 dark:border-stone-600">↓</kbd><span>to navigate</span></div>
              <div className="flex items-center gap-1"><kbd className="rounded border px-1.5 py-0.5 border-stone-300 dark:border-stone-600">Enter</kbd><span>to open</span></div>
            </div>
            <div className="flex items-center gap-1"><kbd className="rounded border px-1.5 py-0.5 border-stone-300 dark:border-stone-600">Esc</kbd><span>to close</span></div>
          </div>
        </Cmdk>
      </div>
    </div>
